...
Typically, we store this mapping file on the configuration repo under an 'anonymization' directory then manyally copy it
to the v1 test server when you run the anonymization script.
   
Subforms
--------
To handle subform fields that need to anonymized, first add a hash entry for the subform itself then one for each
field on that subform that needs to be anonymized.
Example:
```
   ...
